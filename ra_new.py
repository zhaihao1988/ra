...
from scipy.special import gamma as gamma_function, psi, polygamma # psi and polygamma for t dist moments if needed
...
def get_distribution_mode(dist_obj, params, data_min, data_max):
    def neg_pdf(x, dist, p_args):
        ...
        try:
            if dist.name == 'genextreme' and len(p_args)==3:
                ...
                return -dist.pdf(x, loc=loc, scale=scale)
        except: # Broad except for numerical issues with pdf at certain x for some params
            return np.inf # Return a large number if pdf fails, so optimizer avoids it
    ...
    try:
        ...
        if dist_obj.name == 't': # For t-distribution, mode is loc
            return params[1] # loc
        if dist_obj.name == 'pareto': # For pareto, mode is at its lower bound (loc + scale if loc is offset)
                                     # Scipy's pareto loc is an offset, scale is xm. Mode is at xm.
            return params[1] + params[2] if len(params)==3 else params[0] # loc + scale or just scale if loc=0 (fit might fix loc)
...
def calculate_distribution_mean(dist_obj, params):
    try:
        if dist_obj.name == 'genextreme':
            ...
            # params for t are (df, loc, scale)
            df = params[0]
            if df > 1:
                return dist_obj.mean(*params) # loc
            else:
                print(f"警告: t分布自由度 df={df} (<=1)，均值不存在或为无穷。返回np.nan。")
                return np.nan
        elif dist_obj.name == 'pareto':
            # params for pareto are (b, loc, scale) where b is shape, scale is xm
            b = params[0]
            if b > 1:
                return dist_obj.mean(*params) # loc + scale * b / (b - 1)
            else:
...
# 主函数 (其他部分与上一版本相同，只修改distributions_to_try)
def calculate_lrc_ra_ratio(historical_cor_data,
                           expected_cor_basis='fixed',
                           fixed_expected_cor_value=0.95,
                           confidence_level=0.75,
                           manual_dist_choice=None):
    data = np.array(historical_cor_data)
    if not np.all((data > 0)): # 确保所有数据为正，对于lognorm, gamma, weibull_min, pareto
    ...
    for distribution in distributions_to_try:
        ...
        try:
            ...
            if dist_name in ['lognorm', 'gamma', 'weibull_min', 'pareto'] and not np.all(data > 0):
                ...
                continue
            # Pareto分布的loc通常是数据的最小值，或者由fit方法确定
            # 如果我们想让pareto的loc固定为0，scale代表xm，需要特殊处理fit
            # 这里我们先用标准fit，它可能会估计loc和scale
            # 对于pareto，fit返回的参数是 b (shape), loc, scale (xm)

            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                # 对于pareto，fit可能会把loc设为min(data)-epsilon，scale设为一个很小的值，b主导形状
                # 如果数据中有远小于1的值，Pareto的loc可能会估计为负，导致pdf在某些正数点无定义
                # 为了更稳定地拟合Pareto，通常假设loc=0，scale=min(data)，只拟合形状参数b
                # 但scipy的fit不直接支持这种约束。我们先用标准fit。
                if dist_name == 'pareto':
                    # 尝试固定loc为0，让scale作为xm，只拟合形状参数b
                    # 这需要自定义拟合过程，或者接受标准fit返回的loc, scale, b
                    # 为了简单起见，我们继续使用标准fit，但要注意其参数解释
                    # 标准Pareto I: PDF(x, b) = b * x_m^b / x^(b+1) for x >= x_m
                    # Scipy: pareto.pdf(x, b, loc, scale) where x_m = scale, and x_shifted = (x - loc)
                    # So, (x-loc) >= scale. Or x >= loc + scale. min_val = loc + scale
                    # `fit`估计的是b, loc, scale
    ...
    if manual_dist_choice and best_fit_distribution is None:
        ...
        if not selected_params_found:
             print(f"\n错误：用户指定的手动选择分布 '{manual_dist_choice}' 虽然在尝试列表中，但未能成功拟合有效参数。")
             return None
    ...
    data_plot_min = min(data) * 0.8 if min(data) > 0 else (min(data) - 0.1 if min(data) <=0 else 0.01) # 调整绘图下限
    ...
    if best_fit_distribution.name in ['lognorm','gamma','weibull_min', 'pareto', 'genextreme']:
        ...
        # 如果loc本身为负，但分布只在正数有意义（如lognorm），则从一个很小的正数开始
        if current_loc <= 0 and best_fit_distribution.name in ['lognorm','gamma','weibull_min', 'pareto']:
            data_plot_min = 1e-9
    if best_fit_distribution.name == 'weibull_min' and len(best_params)>=3 and best_params[0] < 1: # c, loc, scale
        data_plot_min = best_params[1] # loc
    ...
    # 特定分布的形状参数处理 (scipy中fit返回的参数顺序通常是 shapes, loc, scale)
    if best_fit_distribution.name in ['genextreme', 'weibull_min', 'gamma', 'lognorm', 't', 'pareto']:
        if len(best_params) >=1 : # 至少有一个形状参数
             shape_params_for_probplot = best_params[0] if len(best_params) ==1 else best_params[:-2] if len(best_params) >2 else best_params[0] # 简单处理
             if best_fit_distribution.name == 't': # t(df, loc, scale) -> df是形状
                 shape_params_for_probplot = best_params[0]
             elif best_fit_distribution.name == 'pareto': # pareto(b, loc, scale) -> b是形状
                  shape_params_for_probplot = best_params[0]
    ...
    if expected_cor_basis == 'fixed':
        ...
        print(f"RA计算基准 (COR_expected): 固定值 {expected_cor_to_use:.4f} ({expected_cor_to_use*100:.2f}%)")
    elif expected_cor_basis == 'mean':
        expected_cor_to_use = calculate_distribution_mean(best_fit_distribution, best_params)
        print(f"RA计算基准 (COR_expected): 拟合分布的均值 {expected_cor_to_use:.4f} ({expected_cor_to_use*100:.2f}%)")
        ...
        print(f"RA计算基准 (COR_expected): 拟合分布的众数 (峰值) {expected_cor_to_use:.4f} ({expected_cor_to_use*100:.2f}%)")
    else:
    ...
    if expected_cor_to_use is not None and not np.isnan(expected_cor_to_use) and expected_cor_to_use > 0 :
        ...
        print(f"警告: 计算得到的预期综合成本率 ({expected_cor_to_use}) 无效 (零、负或NaN)，RA比率无法计算。")
...
# --- 主程序执行 (与上一版本相同，但 calculate_lrc_ra_ratio 内部的 distributions_to_try 已更新) ---
if __name__ == "__main__":
    """
    historical_cors_real = [
        0.828094482, 0.543436579, 0.833697162, 1.265072284, 0.975212324,
        0.729494495, 0.894991516, 0.732280431, 0.798998131, 0.714633169
    ]

    historical_cors_real = [1.496343733, 1.336118436, 0.70185022, 0.82899037, 0.602292538,

    0.824430755, 0.641732692, 0.68410927, 0.772624554, 0.746709797
    ]
     """
    historical_cors_real =[0.425299364, 0.596419551, 0.560003463, 0.605537298, 0.668452032,
        0.777330604, 0.876000948, 0.796779993, 0.664239748, 0.455203044
    ]
    print("开始计算LRC RA比率 (使用真实数据，增加t和pareto分布)...")
    print(f"历史综合成本率数据: {historical_cors_real}")
    print("-" * 50)

    real_data_mean = np.mean(historical_cors_real)
    print(f"(参考：真实数据的均值为: {real_data_mean:.4f})")
    fixed_cor_for_scenario1 = real_data_mean # 使用真实数据均值作为固定预期

    print(f"\n--- 场景1: 使用固定的预期COR ({fixed_cor_for_scenario1*100:.2f}%), 自动选择分布 ---")
    results_fixed_auto = calculate_lrc_ra_ratio(
        historical_cors_real,
        expected_cor_basis='fixed',
        fixed_expected_cor_value=fixed_cor_for_scenario1,
        manual_dist_choice='t'
    )
    if results_fixed_auto:
        print("\n场景1 - 结果总结:")
        print(f"  选择的分布 (自动AIC): {results_fixed_auto['selected_distribution_name']}")
        print(f"  75%分位COR: {results_fixed_auto['cor_at_confidence_level']:.4f}")
        print(f"  预期COR (固定值): {results_fixed_auto['expected_cor_used_for_ra']:.4f}")
        print(f"  LRC RA 比率: {results_fixed_auto['lrc_ra_ratio']:.4f} ({results_fixed_auto['lrc_ra_ratio']*100:.2f}%)")

    print(f"\n--- 场景2: 使用拟合分布的均值作为预期COR, 手动选择 'genextreme' ---")
    results_mean_manual_gev = calculate_lrc_ra_ratio(
        historical_cors_real,
        expected_cor_basis='mean',
        manual_dist_choice='t'
    )
    if results_mean_manual_gev:
        print("\n场景2 - 结果总结:")
        print(f"  选择的分布: {results_mean_manual_gev['selected_distribution_name']}")
        print(f"  75%分位COR: {results_mean_manual_gev['cor_at_confidence_level']:.4f}")
        print(f"  预期COR (均值): {results_mean_manual_gev['expected_cor_used_for_ra']:.4f}")
        print(f"  LRC RA 比率: {results_mean_manual_gev['lrc_ra_ratio']:.4f} ({results_mean_manual_gev['lrc_ra_ratio']*100:.2f}%)")

    print(f"\n--- 场景3: 使用拟合分布的众数(峰值)作为预期COR, 手动选择 'genextreme' ---")
    results_mode_manual_gev = calculate_lrc_ra_ratio(
        historical_cors_real,
        expected_cor_basis='mode',
        manual_dist_choice='t'
    )
    if results_mode_manual_gev:
        print("\n场景3 - 结果总结:")
        print(f"  选择的分布: {results_mode_manual_gev['selected_distribution_name']}")
        print(f"  75%分位COR: {results_mode_manual_gev['cor_at_confidence_level']:.4f}")
        print(f"  预期COR (众数): {results_mode_manual_gev['expected_cor_used_for_ra']:.4f}")
        print(f"  LRC RA 比率: {results_mode_manual_gev['lrc_ra_ratio']:.4f} ({results_mode_manual_gev['lrc_ra_ratio']*100:.2f}%)")

    if results_mode_manual_gev:
        print("\n--- 最后一次调用的所有分布拟合详情 (场景3 - 手动选GEV) ---")
        print(results_mode_manual_gev['all_fit_results'])